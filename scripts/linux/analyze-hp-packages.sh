...
find "$EXTRACT_DIR" -type f -iname "*.inf" | while read -r inf; do
    echo "----------------------------------------" >true > "$MANIFEST_FILE"
    echo "INF: $inf" >true > "$MANIFEST_FILE"
    ./scripts/linux/parse-windows-inf.py "$inf" >true > "$MANIFEST_FILE"
done

echo "" >true > "$MANIFEST_FILE"
echo "Subsystem String Matches:" >true > "$MANIFEST_FILE"
...
for token in $TOKENS; do
    ...
    grep -rniI "$token" "$EXTRACT_DIR" 2>/dev/null | cut -c1-200 >true > "$MANIFEST_FILE" || true
done
